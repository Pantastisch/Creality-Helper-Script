    ...
    case "${remove_menu_opt}" in
      1)
        if [ ! -d "$MOONRAKER_FOLDER" ] && [ ! -d "$NGINX_FOLDER" ]; then
         ...
         run "remove_moonraker_nginx" "remove_menu_ui_e5m"
        fi;;
      2)
        if [ ! -d "$FLUIDD_FOLDER" ]; then
          ...
          run "remove_fluidd" "remove_menu_ui_e5m"
        fi;;
      3)
        if [ ! -d "$MAINSAIL_FOLDER" ]; then
          ...
          run "remove_mainsail" "remove_menu_ui_e5m"
        fi;;
      4)
        if [ ! -f "$ENTWARE_FILE" ]; then
          ...
          run "remove_entware" "remove_menu_ui_e5m"
        fi;;
      5)
