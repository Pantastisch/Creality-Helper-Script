    ...
    case "${tools_menu_opt}" in
      1)
        if [ -f "$INITD_FOLDER"/disabled.S55klipper_service ]; then
          ...
          run "prevent_updating_klipper_files" "tools_menu_ui_e5m"
        fi;;
      2)
        if [ ! -f "$INITD_FOLDER"/disabled.S55klipper_service ]; then
          error_msg "Updating Klipper configuration files is already allowed!"
        else
          run "allow_updating_klipper_files" "tools_menu_ui_e5m"
        fi;;
      3)
        if [ -f "$KLIPPER_KLIPPY_FOLDER"/gcode.py ]; then
          run "printing_gcode_from_folder" "tools_menu_ui_e5m"
        fi;;
      4)
        if grep -q "^\[webcam Camera\]$" "$MOONRAKER_CFG"; then
          error_msg "Camera settings are alredy enabled in Moonraker!"
        else
          run "enable_camera_settings" "tools_menu_ui_e5m"
        fi;;
      5)
        if grep -q "^#\[webcam Camera\]" "$MOONRAKER_CFG"; then
          error_msg "Camera settings are alredy disabled in Moonraker!"
        else
          run "disable_camera_settings" "tools_menu_ui_e5m"
        fi;;
      6)
        if [ ! -d "$NGINX_FOLDER" ]; then
          error_msg "Nginx is not installed!"
        else
          run "restart_nginx_action" "tools_menu_ui_e5m"
        fi;;
      7)
        if [ ! -d "$MOONRAKER_FOLDER" ]; then
          error_msg "Moonraker is not installed!"
        else
          run "restart_moonraker_action" "tools_menu_ui_e5m"
        fi;;
      8)
        if [ ! -f "$INITD_FOLDER"/S55klipper_service ]; then
          error_msg "Klipper service is not present!"
        else
          run "restart_klipper_action" "tools_menu_ui_e5m"
        fi;;
      9)
        if [ ! -f "$ENTWARE_FILE" ]; then
          error_msg "Entware is not installed!"
        else
          run "update_entware_packages" "tools_menu_ui_e5m"
        fi;;
      10)
        run "clear_cache" "tools_menu_ui_e5m";;
      11)
        run "clear_logs" "tools_menu_ui_e5m";;
      12)
        run "restore_previous_firmware" "tools_menu_ui_e5m";;
      13)
        run "reset_factory_settings" "tools_menu_ui_e5m";;
      B|b)
