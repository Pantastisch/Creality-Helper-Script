    ...
    case "${customize_menu_opt}" in
      1)
        if [ ! -d "$FLUIDD_FOLDER" ] && [ ! -d "$MAINSAIL_FOLDER" ]; then
          ...
          run "remove_creality_web_interface" "customize_menu_ui_e5m"
        fi;;
      2)
        if [ -f "$CREALITY_WEB_FILE" ]; then
          ...
          run "restore_creality_web_interface" "customize_menu_ui_e5m"
        fi;;
      3)
        if [ -d "$GUPPY_SCREEN_FOLDER" ]; then
          ...
          run "install_guppy_screen" "customize_menu_ui_e5m"
        fi;;
      4)
        if [ ! -d "$GUPPY_SCREEN_FOLDER" ]; then
          error_msg "Guppy Screen is not installed!"
        else
          run "remove_guppy_screen" "customize_menu_ui_e5m"
        fi;;
      5)
